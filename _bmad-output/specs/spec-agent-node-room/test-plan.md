...
- a **read-only replay script** queries the deployment DB for the distinct tool names + their row counts, runs the resolver over them, and reports the generic-fallback **numerator** (rows resolving to `generic`) and **denominator**, plus the fraction
...
- **denominator conflict, recorded not settled:** this section historically reads "total rows" (raw storage rows), but paired call/result storage means raw rows double-count modern calls. The recommended alternative is one **logical UI tool card per invocation**, projected through `projectToolTranscript()`. The owner-ratified choice — and the raw-row alternative if retained — is decided in `phase-03-audit-gate-and-verification.md`; until ratification no denominator is implemented
