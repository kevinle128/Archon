...
- `develop` is the working branch. All feature work branches off `develop` and merges back into `develop`.
...
- To release, use the `/release` skill. It compares `develop` to `main`, generates changelog entries, bumps the version, and creates a PR to merge `develop` into `main`.
