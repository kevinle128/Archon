...
describe('ConsoleNodeRoom', () => {
  ...
  describe('todo strip', () => {
    ...
    test('room-region direct children are scroller → controls → todo → dock with queue before composer', async () => {
      ...
      const bandMessages: WorkflowNodeMessage[] = [
        {
          id: 'status-1',
          seq: 1,
          kind: 'status',
    ...
    test('absent bands leave no empty slot and no unrequested steering action', async () => {
      ...
      await flushUntil('live single-occurrence', () => dockField() !== null);
      ...
      expect(
        Array.from(region.querySelectorAll('button')).some(button =>
          (button.textContent ?? '').includes('Jump to latest')
        )
      ).toBe(false);
      ...
      expect(region.getAttribute('class') ?? '').not.toContain('m-[4px]');
      ...
      const jump = Array.from(region.querySelectorAll('button')).find(button =>
        (button.textContent ?? '').includes('Jump to latest')
      );
      ...
      await flushUntil('terminal no dock', () => (host.textContent ?? '').includes('first'));
      region = host.querySelector('[role="region"][aria-label="review room"]');
      if (region === null) throw new Error('missing terminal room region');
      children = Array.from(region.children);
      const terminalScroller = host.querySelector('[data-testid="console-node-room-scroll"]');
      ...
      await act(async () => {
        renderRoom({
          ...
          definitionNodes: [{ id: 'loop', prompt: 'Loop' }],
      ...
      await flushUntil('finished queue', () => (host.textContent ?? '').includes('queued · 1'));
      expect(dockField()).toBeNull();
