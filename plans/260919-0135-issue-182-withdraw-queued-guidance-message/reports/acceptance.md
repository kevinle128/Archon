...
| `bun run --cwd e2e test:ui -- --grep 'withdraw'`                 | 5 passed (2.9m)                                                                                                                                                                                                           |
| `bun run --cwd e2e test:ui -- --grep 'withdraw\|queue guidance'` | 16 passed (6.8m) — 5 new withdraw + 11 Story 2.1 regression (agent-queue-guidance)                                                                                                                                        |
...
- Accessible names observed (measurements): `delete · <full message text>` — e.g. `delete · WITHDRAWN-ce70c7ec7019 obsolete operator instruction padding …` (Console) and `delete · <<E2E_SCENARIO>>{"echoPrompt":true}<</E2E_SCENARIO>>SECOND-83b32de5aa68` (Legacy sibling). Button `type="button"` on both surfaces.
- Measured target size: Console **55.125 × 24 px**; Legacy **55.609 × 24 px** — both ≥ 24 × 24.
- Focused focus-ring computed values (via real Shift+Tab traversal, so `:focus-visible` applied): Console `2px solid oklab(0.64 0.255478 -0.1475 / 0.710281)`; Legacy `2px solid oklab(0.672201 -0.0615636 -0.169145 / 0.594232)`.
...
The final review reran that 19-case suite after moving the method-scoped withdraw authentication middleware ahead of the install-wide `/api/*` gate. Its unauthenticated invalid-UUID case now exercises the default enabled gate and proves the steering route's nested 401 contract wins over both the generic gate response and OpenAPI validation.
