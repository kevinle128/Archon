...
| Safe boundary `M`    | Production-observable; current prompt and inherited context survive                   | **SUPERSEDED:** the prior Node-spawn observation is not valid production-spawn evidence. Re-run the full current-Bun text/reasoning/tool matrix after the tree-reap defect is resolved.                     |
| Stop acknowledgement | `< 1,000 ms` in every cold/warm sample on each host                                   | **SUPERSEDED:** the prior Node-spawn latency samples cannot certify the production Bun path. Re-run all new/resumed/forked samples on each required native host.                                            |
| Fresh turn           | Assigned id resumes with current marker                                               | **SUPERSEDED:** re-run B0/B2 through the current Bun-owned-tree runner.                                                                                                                                     |
| Resumed turn         | Same id resumes with old + current markers; immediate-stop policy decided by evidence | **SUPERSEDED:** re-run B3 through the current Bun-owned-tree runner.                                                                                                                                        |
| Forked turn          | Assigned fork id resumes with source + fork markers; source remains unchanged         | **SUPERSEDED:** re-run B4 through the current Bun-owned-tree runner.                                                                                                                                        |
| POSIX process tree   | Exact child gone after graceful Stop/Cancel                                           | **BLOCKED:** corrected native-macOS Bun runs B7/B9 settle in 120/94 ms but exact descendants remain alive. See `plans/reports/spike-260920-1433-grok-interrupt-resume-audit-rerun.md`.                      |
...
| Grace interval       | Measured graceful shutdown bound with margin below 1,000 ms total                     | Corrected B7/B9 Stop-to-settlement is 120/94 ms without SIGKILL, but tree cleanup still fails. The 800 ms interrupt and 5000 ms Cancel grace are candidate values only until the full native matrix passes. |
