...
## Finding 5: Plain-text normalization path (ANSI-strip / passthrough) has no stated bound on the *input* size, only on the *displayed* output — a second unbounded main-thread path for providers that don't truncate at rest
...
- **Suggested fix:** Apply the head-slice (or an input-size guard) *before* running `stripAnsi`/line-splitting, not just before display, so the bound actually limits the amount of adversarial/large text processed rather than only the amount shown.
